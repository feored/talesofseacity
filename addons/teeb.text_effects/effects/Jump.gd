...
# Syntax: [jump angle=3.141 timeoffset =1][/jump]
var bbcode = "jump"
...
func _process_custom_fx(char_fx):
    ...
    var timeoffset = char_fx.env.get("timeoffset", 0)
    var t = abs(sin((char_fx.elapsed_time + timeoffset) * 8.0 + _w_char * PI * .025)) * 4.0
