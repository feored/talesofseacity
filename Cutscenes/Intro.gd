...
func showCharacter(character : String, show : bool):
	...
	tween.tween_property(vn_characters[character], "modulate", Color.white if show else Color.transparent, SHOW_TIMER)
	tween.tween_callback(self, "next")

func flipCharacter(character : String, flip : bool):
	var tween = create_tween()
	var characterSprite = vn_characters[character]
	tween.tween_property(characterSprite, "scale", Vector2(-1 * characterSprite.scale.x, characterSprite.scale.y), SHOW_TIMER)
	tween.tween_callback(self, "next")
